<?php
     ...
     * @method $this and (mixed $condition, array $parameters = []) Add AND condition
     * @method $this or (mixed $condition, array $parameters = []) Add OR condition
     */
    class Result extends NotORMBase implements Iterator, ArrayAccess, Countable, JsonSerializable
    {
            ...
            if ($this->notORM->driver == 'mysql') {
                ...
                try {
                    ...
                    if ($errorMode == PDO::ERRMODE_EXCEPTION) {
                        ...
                        @trigger_error('PDOStatement::execute(): '.$e->getMessage(), E_USER_WARNING);
                    }
                }
            }
        }
        ...
        protected function quote($val)
        {
            ...
            if (is_int($val) || $val instanceof Literal) {
                return (string)$val;
            }
         ...
         *
         * @return mixed inserted Row or false in case of an error or number of affected rows for INSERT ...
         *               SELECT
         */
        public function insert($data)
        {
            ...
            if ($this->notORM->driver == 'pgsql') {
                if ( ! isset($data[$this->primary])) {
                    //获取序列名称
                    $pgss = $this->query(
                        "SELECT pg_get_serial_sequence('".$this->table."', '".$this->primary."') pgss",
                        $this->parameters
                    )->fetch();
                    if (isset($pgss['pgss'])) {
                        $rs                   = $this->query(
                            "select last_value id from ".$pgss['pgss'],
                            $this->parameters
                        )->fetch();
        ...
        public function query($query, $parameters)
        {
             ...
             */
            $parameters = array_map([$this, 'formatValue'], $parameters);
            ...
            if ( ! $return || ! $return->execute()) {
                ...
                //修复一个语法错误
                $errorMessage = isset($errorInfo[2]) ? $errorInfo[2] : null;
            ...
            if ($this->notORM->debug) {
                ...
                echo sprintf(
                    "[%s - %ss]%s<br>\n",
                    self::$queryTimes,
                    round($debugTrace['endTime'] - $debugTrace['startTime'], 5),
                    $debugTrace['sql']
                );
            }
        ...
        protected function whereOperator($operator, array $args)
        {
            ...
            } elseif ($parameters instanceof Result) {
         ...
         *
         * @param string $columns for example "column, MD5(column) AS column_md5", empty string to reset previously set
         *                        columns
         * @param        string   ...
         *
        ...
        public function delete()
        {
            ...
            $return = $this->query(
                "DELETE".$this->topString($this->limit, $this->offset)." FROM $this->table".$where,
                $this->parameters
            );
            if ( ! $return) {
                return false;
            ...
            foreach ($matches[1] as $names) {
                ...
                if ($names != "$parent.") {
                    ...
                    foreach ($matches as $match) {
                        ...
                        // should use alias if the table is used on more places
                        $return[$name] = " LEFT JOIN $table".($table != $name ? " AS $name"
                                : "")." ON $parent.$column = $name.$primary";
                        $parent        = $name;
                    }
                }
            }
         ...
         * @return mixed
         */
        public function fetchRow($column = '')
        {
        ...
        public function __toString()
        {
            ...
            $join   = $this->createJoins(
                implode(",", $this->conditions).",".implode(",", $this->select).",$this->group,$this->having,".implode(
                    ",",
                    $this->order
                )
            );
        ...
        /**
         * queryRows别名，等效于Result::queryRows($sql, $parmas)
         * @author: dogstar 2015-04-26
         *
         * @param       $sql
         * @param array $parmas
         ...
         * @return array
         */
        public function queryAll($sql, $parmas = [])
        {
            return $this->queryRows($sql, $parmas);
        }
        
        /**
         * 根据SQL查询全部数据，等效于Result::queryAll($sql, $parmas)
         *
         * @param       $sql
         * @param array $parmas
         *
         ...
         * @author: dogstar 2014-11-01
         */
        public function queryRows($sql, $parmas = [])
        {
