<?php
	 ...
	 */
	class Result extends Base implements Iterator, ArrayAccess, Countable, JsonSerializable {
		 ...
		 *
		 * @param mixed                             array($column => $value)|Traversable for single row insert or
		 *                                                        Result|string for INSERT
		 *                                          ... SELECT
		...
		public function query( $query, $parameters ) {
			...
			if ( ! $return || ! $return->execute() ) {
				...
				$errorMessage = isset( $errorInfo[2] ) ? $errorInfo[2] : null;//修复一个语法错误
